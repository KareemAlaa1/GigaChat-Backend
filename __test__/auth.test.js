...
describe('auth', () => {
    ...
    describe('ExistedEmailORusername endpoint', () => {
        it('should respond with 200 and message if email exists', async () => {
            ...
            const existingUser = await User.create({ email: 'existinguser@example.com',username: 'existingusername', active: true });
        ...
        it('should respond with 200 and message if username exists', async () => {
            ...
            const existingUser = await User.create({ username: 'existingusername',email:'temp@email.com', active: true });
        ...
        it('should respond with 400 if neither email nor username is provided', async () => {
            ...
            expect(response.status).toBe(400);
            expect(response.body.error).toBe('Email or username is required in the request body');
        });

        it('should respond with 404 if email and username do not exist', async () => {
            const response = await request(app)
                .post('/api/user/ExistedEmailORusername')
                .send({ email: 'nonexistent@example.com', username: 'nonexistentuser' });
